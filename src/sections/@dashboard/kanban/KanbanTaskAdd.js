...
export function useDatePicker({ date }) {
  const [dueDate, setDueDate] = useState([date && date[0] ? date[0] : '', date ? date[1] : '']);
  const [openPicker, setOpenPicker] = useState(false);

  const startTime = dueDate && dueDate.length > 0 ? dueDate[0] : '';
  const endTime = dueDate && dueDate.length > 1 ? dueDate[1] : '';
